...
; 1.2.2. It would be nice to be able to easily change the currency used
...
; a list of the values of the coins to use rather than an integer specify-
; ing which coins to use. We could then have lists that defined each
; kind of currency:
...
; still have the same form, but it will access its second argument dif-
; ferently, as follows:
          ...
          (+ (cc amount
             ...
             (cc (- amount
                    (first-denomination
                      coin-values))
                 coin-values)))))
; Define the procedures first-denomination, except-first-de-
; nomination, and no-more? in terms of primitive operations on list
